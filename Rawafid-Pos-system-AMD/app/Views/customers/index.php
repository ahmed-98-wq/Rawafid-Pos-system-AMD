...
<div class="card" style="padding:0;overflow:hidden">
  <div class="table-wrap">
    <table class="data-table" id="ct">
      <thead><tr>
        <th>العميل</th><th>الهاتف</th><th>المشتريات</th><th>الفواتير</th><th>الرصيد</th><th>آخر فاتورة</th><th style="width:110px">إجراءات</th>
      </tr></thead>
      <tbody>
      <?php if(empty($customers)): ?>
        <tr><td colspan="7" class="empty-td">
          <div style="text-align:center;padding:30px;color:var(--text-3)">
            <i class="ti ti-users-off" style="font-size:40px;display:block;margin-bottom:10px;opacity:.35"></i>
            <?= $search ? "لا نتائج لـ «".Helper::e($search)."»" : "لا يوجد عملاء بعد" ?>
            <?php if(!$search): ?><br><button class="btn btn-primary btn-sm" style="margin-top:10px" onclick="openCustomerModal()"><i class="ti ti-plus"></i> أضف عميل</button><?php endif; ?>
          </div>
        </td></tr>
        ...
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              <div class="c-avatar" style="background:<?= cColor($c['name']) ?>"><?= mb_strtoupper(mb_substr($c['name'],0,1)) ?></div>
              <div>
                <div style="font-weight:600;font-size:13px;color:var(--text)"><?= Helper::e($c['name']) ?></div>
                <?php if($c['address']): ?><div style="font-size:11px;color:var(--text-3)"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-map-pin" width="14" height="14" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
  ...
  <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z" />
          ...
          <td><?= $c['phone'] ? '<a href="tel:'.Helper::e($c['phone']).'" style="color:var(--olive-600);text-decoration:none;font-size:12px"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-phone" width="14" height="14" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
  <path d="M5 4h4l2 5l-2.5 1.5a11 11 0 0 0 5 5l1.5 -2.5l5 2v4a2 2 0 0 1 -2 2a16 16 0 0 1 -15 -15a2 2 0 0 1 2 -2" />
</svg> '.Helper::e($c['phone']).'</a>' : '<span style="color:var(--text-3)">—</span>' ?></td>
          <td class="fw-bold"><?= number_format($c['total_purchases'],2) ?> <small style="color:var(--text-3)"><?= $currency ?></small></td>
          <td style="text-align:center"><span style="background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:2px 8px;font-size:12px"><?= $c['invoice_count'] ?></span></td>
          <td>
            <?php if($c['balance']>0): ?>
              <div style="display:flex;align-items:center;gap:6px">
                <span class="badge badge-danger"><?= number_format($c['balance'],2) ?> <?= $currency ?></span>
                <button onclick="openPayModal(<?=$c['id']?>,'<?=Helper::e($c['name'])?>',<?=$c['balance']?>)" style="background:none;border:none;color:var(--olive-500);cursor:pointer;font-size:15px" title="تسجيل دفعة"><i class="ti ti-cash"></i></button>
              </div>
            <?php else: ?>
  ...
  <path d="M5 12l5 5l10 -10" />
          ...
          <td style="font-size:12px;color:var(--text-3)"><?= $c['last_invoice'] ? Helper::formatDate($c['last_invoice'],'d/m/Y') : '—' ?></td>
          <td>
  ...
  <div class="pagination" style="padding:12px 16px">
    <span style="color:var(--text-3);font-size:12px;margin-left:auto">عرض <?=($offset+1)?>–<?=min($offset+$perPage,$total)?> من <?=$total?></span>
...
<div class="modal-overlay" id="cust-modal" style="display:none">
  ...
  <path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" />
          ...
          <input type="text" name="name" id="c-name" class="form-input" style="font-size:15px;padding:11px 14px" placeholder="الاسم الكامل" required oninput="updateCmIco(this.value)">
          <div id="e-name" style="font-size:11px;color:#a32d2d;margin-top:3px;display:none"></div>
        </div>
        <div class="form-row">
          <div class="form-group">
  ...
  <path d="M17.657 16.657l-4.243 4.243a2 2 0 0 1 -2.827 0l-4.244 -4.243a8 8 0 1 1 11.314 0z" />
      ...
      <div class="cm-panel" id="t-finance">
        ...
        <div id="fin-info" style="display:none;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:12px;margin-bottom:14px">
          <div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text-2);padding:3px 0"><span>إجمالي المشتريات</span><strong id="fi-total">—</strong></div>
          <div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text-2);padding:3px 0"><span>عدد الفواتير</span><strong id="fi-count">—</strong></div>
          <div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text-2);padding:3px 0;border-top:1px solid var(--border);margin-top:4px;padding-top:8px"><span>الرصيد المستحق</span><strong id="fi-bal" style="color:#a32d2d">—</strong></div>
        </div>
  ...
  <path d="M11 15l3 0" />
        ...
        <div id="adj-section" style="display:none">
          <div style="display:flex;align-items:center;gap:8px;margin:10px 0;padding:10px;background:var(--bg);border-radius:8px">
            <div style="flex:1;font-size:13px;color:var(--text-2)">الرصيد الحالي</div>
            <div id="cur-bal-disp" style="font-size:18px;font-weight:700;color:var(--text)">—</div>
          </div>
          <label style="display:flex;align-items:center;gap:8px;font-size:13px;color:var(--text-2);cursor:pointer;margin-bottom:10px">
            <input type="checkbox" name="adjust_balance" id="adj-cb" onchange="toggleAdj(this.checked)" style="accent-color:var(--olive-500)">
            تعديل الرصيد يدوياً
          </label>
          <div id="adj-inputs" style="display:none">
            <div class="form-group">
              ...
              <small class="form-hint" style="color:#854f0b">⚠️ سيتم تعديل الرصيد مباشرة</small>
            </div>
          </div>
        </div>
  ...
  <path d="M9 15l4 0" />
        ...
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:6px">
          <span style="font-size:11px;color:var(--text-3)">إضافة سريعة:</span>
          <?php foreach(['عميل VIP','لا يُباع بالآجل','يطلب فاتورة رسمية','توصيل للمنزل','خصم خاص'] as $n): ?>
          <button type="button" onclick="addNote('<?= $n ?>')" style="background:var(--bg);border:1px solid var(--border);border-radius:14px;padding:3px 10px;font-size:11px;cursor:pointer;color:var(--text-2)"><?= $n ?></button>
          <?php endforeach; ?>
        </div>
      </div>
  ...
  <div class="modal-box" style="max-width:400px">
    <div class="modal-header">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div><h3>تسجيل دفعة</h3><p id="pm-name" style="font-size:12px;color:var(--text-3)">—</p></div>
      </div>
      <button onclick="closePayModal()" style="background:none;border:none;cursor:pointer;color:var(--text-3);font-size:20px"><i class="ti ti-x"></i></button>
    </div>
    <form method="POST" action="?page=customers&action=payment">
      <input type="hidden" name="csrf_token" value="<?= Session::csrf() ?>">
      <input type="hidden" name="customer_id" id="pm-id">
      <div style="background:#fff5f5;border:1px solid #f7c1c1;border-radius:8px;padding:12px;margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:13px"><span>الرصيد المستحق</span><strong id="pm-debt" style="color:#a32d2d">—</strong></div>
      </div>
      <div class="form-group">
        <label class="form-label required">مبلغ الدفعة (<?= $currency ?>)</label>
        <input type="number" name="payment_amount" id="pm-amount" class="form-input" style="font-size:16px;padding:12px" step="0.01" min="0.01" required placeholder="0.00" oninput="calcRem()">
      </div>
      <div id="rem-box" style="display:none;background:#f0faf0;border:1px solid #b3e0b3;border-radius:8px;padding:10px;margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;font-size:13px"><span>المتبقي بعد الدفعة</span><strong id="rem-val">—</strong></div>
      </div>
      <div style="display:flex;gap:8px;margin-bottom:14px">
        <button type="button" onclick="setFull()" style="flex:1;padding:7px;border:1px solid var(--olive-300);border-radius:7px;background:var(--olive-50);color:var(--olive-700);font-size:12px;font-weight:600;cursor:pointer">دفع الكل</button>
        <button type="button" onclick="setHalf()" style="flex:1;padding:7px;border:1px solid var(--olive-300);border-radius:7px;background:var(--olive-50);color:var(--olive-700);font-size:12px;font-weight:600;cursor:pointer">نصف المبلغ</button>
      </div>
...
<!-- Styles -->
<style>
.c-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px}
.c-stat{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:16px;display:flex;align-items:center;gap:12px}
.cs-ico{width:44px;height:44px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0}
.c-stat.blue .cs-ico{background:#e6f1fb;color:#185fa5}
.c-stat.red  .cs-ico{background:#fcebeb;color:#a32d2d}
.c-stat.orange .cs-ico{background:#faeeda;color:#854f0b}
.cs-v{font-size:22px;font-weight:700;color:var(--text)}
.cs-l{font-size:11px;color:var(--text-2);margin-top:2px}
.s-wrap{position:relative;display:flex;align-items:center;flex:1;min-width:200px}
.s-wrap > i{position:absolute;right:10px;color:var(--text-3);font-size:15px;pointer-events:none}
.si{padding-right:34px;flex:1}
.s-clear{position:absolute;left:8px;color:var(--text-3);font-size:14px;cursor:pointer;text-decoration:none}
.c-avatar{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:14px;font-weight:700;flex-shrink:0}
.cm-box{width:540px;max-width:98vw;max-height:92vh;overflow-y:auto}
.cm-ico{width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#4e6b2b,#9ab866);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:white;flex-shrink:0}
.cm-tabs{display:flex;gap:2px;background:var(--bg);border-radius:8px;padding:4px;margin-bottom:18px}
.cm-tab{flex:1;padding:7px 8px;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;color:var(--text-2);background:none;display:flex;align-items:center;justify-content:center;gap:4px;transition:.15s}
.cm-tab.active{background:var(--surface);color:var(--olive-600);box-shadow:0 1px 4px rgba(0,0,0,.08)}
.cm-panel{display:none}
.cm-panel.active{display:block}
@media(max-width:600px){.c-stats{grid-template-columns:1fr}.cm-box{width:100vw}}
</style>
...
function cColor(n) {
  const c=['#4e6b2b','#185fa5','#854f0b','#534ab7','#1a6b6b','#6b1a1a'];
...
async function openCustomerModal(id=null) {
  ...
  if (id) {
    ...
    try {
      ...
      document.getElementById('cur-bal-disp').style.color = bal>0?'#a32d2d':'#3b6d11';
      ...
      document.getElementById('fi-bal').style.color = bal>0?'#a32d2d':'#3b6d11';
    } catch(e){}
  } else {
...
function calcRem() {
  ...
  document.getElementById('rem-val').style.color=r===0?'#3b6d11':(r<debt?'#854f0b':'#a32d2d');
...
function cColor(string $name): string {
    $colors = ['#4e6b2b','#185fa5','#854f0b','#534ab7','#1a6b6b','#6b1a1a','#7a4aae'];
    $hash = 0;

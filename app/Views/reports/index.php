...
<div class="card filters-bar">
  ...
  <path d="M12 15v3" />
      ...
      <input type="date" name="from" id="rpt-from" value="<?= $from ?>" class="form-input w-150">
    </div>
    <div class="filter-group">
      <label class="form-label-sm">إلى</label>
      <input type="date" name="to" id="rpt-to" value="<?= $to ?>" class="form-input w-150">
    </div>
    <div class="filter-group u-style-121">
...
<div class="stats-grid">
  ...
  <div class="stat-card red clickable" onclick="window.location.href='?page=returns'">
  ...
  <path d="M5 10h11a4 4 0 1 1 0 8h-1" />
    ...
    <div class="stat-body">
      ...
      <div class="stat-sub"><?= $currency ?> — <span class="u-style-122">عرض المرتجعات</span></div>
    </div>
  </div>
</div>
...
<div class="row-2col">
  ...
  <path d="M4 15l4 -6l4 2l4 -5l4 4" />
    ...
    <canvas id="daily-chart" class="u-style-123"></canvas>
  </div>
  <div class="card">
    <div class="card-header">
  ...
  <path d="M15 3.5a9 9 0 0 1 5.5 5.5h-4.5a4 4 0 0 0 -1 -1v-4.5z" />
    ...
    <canvas id="pm-chart" class="u-style-123"></canvas>
  </div>
</div>
...
<div class="row-2col">
  ...
  <path d="M17 6v3c0 2.4 -1 4 -3 4" />
    ...
    <?php if (empty($topProducts)): ?>
      <div class="empty-compact">لا توجد بيانات للفترة المحددة</div>
    <?php else: ?>
      <div class="table-wrap table-scroll-md">
        <table class="data-table">
          ...
          <tr>
            ...
            <td><?= number_format($p['revenue'], 2) ?> <small class="text-muted"><?= $currency ?></small></td>
            <td>
              <div class="rpt-profit-row">
                <span class="text-green"><?= number_format($p['profit'], 2) ?></span>
                <div class="mini-bar-wrap"><div class="mini-bar" data-width="<?= min($pct,100) ?>"></div><small><?= $pct ?>%</small></div>
              </div>
            </td>
          </tr>
          <?php endforeach; ?>
    ...
    <div class="table-wrap table-scroll-md">
      <table class="data-table">
        ...
        <tr>
          ...
          <td class="cell-soft"><?= Helper::e($inv['cname'] ?? 'نقدي') ?></td>
...
function buildCharts() {
  ...
  const c1 = cv(isDark ? '--color-hex-5a9eff' : '--color-hex-4a8a3c');
  const c2 = cv(isDark ? '--color-rgb-90-158-255-p25' : '--color-rgb-58-112-48-p2');
  const cArr = isDark
    ? ['--color-hex-5a9eff','--color-hex-72c870','--color-hex-ffca6a','--color-hex-ff8a88','--color-hex-a090ff'].map(cv)
    : ['--color-hex-4a8a3c','--color-hex-84bc72','--color-hex-b87000','--color-hex-b83232','--color-hex-7040a0'].map(cv);
  const tc = cv(isDark ? '--color-hex-4a6888' : '--color-hex-7a8e72');
  ...
  (function() {
    ...
    ctx.fillStyle = cv(isDark ? '--surface' : '--card'); ctx.fill();
    names.forEach((l,i) => {
      const y=28+i*24;
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.mini-bar[data-width]').forEach(bar => {

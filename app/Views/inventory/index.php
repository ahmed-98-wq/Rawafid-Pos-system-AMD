...
<div class="card filters-bar">
  <form method="GET" class="filters-form" id="inv-filter-form">
    ...
    <div class="u-style-43">
      <i class="ti ti-search u-style-44"></i>
      <input type="text" name="search" value="<?= Helper::e($search) ?>"
             placeholder="بحث بالاسم..." class="form-input" 
    ...
    <table class="data-table">
      <thead class="sticky-head">
        <tr>
          ...
          <th>الوحدة</th><th>سعر الشراء</th><th>قيمة المخزون</th><th>الصلاحية</th><th class="w-80">تعديل</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="<?= $isExpired ? 'row-expired' : '' ?>">
          <td>
            <div class="flex items-center gap-2">
              <?php if($isExpired): ?><span title="منتهي الصلاحية" class="u-style-45">⛔</span>
              <?php elseif($isSoon): ?><span title="قريب الانتهاء" class="u-style-46">⏳</span>
              <?php endif; ?>
              <span class="fw-600 <?= $isExpired ? 'text-expired' : '' ?>"><?= Helper::e($p['name']) ?></span>
            </div>
          </td>
          <td class="cell-muted"><?= Helper::e($p['category_name'] ?? '—') ?></td>
          <td><span class="stock-badge <?= $stockCls ?>"><?= number_format($p['stock_qty'], 2) ?></span></td>
          <td class="cell-muted"><?= number_format($p['min_stock_alert'], 0) ?></td>
          <td class="cell-regular"><?= Helper::e($p['unit']) ?></td>
          <td class="cell-regular"><?= number_format($p['purchase_price'], 2) ?> <small class="text-muted"><?= $currency ?></small></td>
          <td class="u-style-47"><?= number_format($p['stock_qty'] * $p['purchase_price'], 2) ?> <small class="text-muted"><?= $currency ?></small></td>
          <td>
...
<!-- Modal: تعديل المخزون -->
<div class="modal-overlay is-hidden" id="adj-modal">
  <div class="modal-box u-style-49">
    <div class="modal-header">
      <div class="modal-title-group">
        <div class="u-style-50">📦</div>
        <div><h3>تعديل المخزون</h3><p id="adj-prod-name" class="cell-muted">اختر منتجاً</p></div>
      </div>
      <button onclick="closeAdj()" class="icon-btn"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-x" width="14" height="14" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
  <path d="M18 6l-12 12" />
  <path d="M6 6l12 12" />
</svg></button>
    ...
    <div id="adj-search-wrap" class="u-style-51">
      <input type="text" id="adj-search" class="form-input" placeholder="ابحث عن منتج..." oninput="adjSearch(this.value)">
      <div id="adj-search-results" class="u-style-52"></div>
    </div>

    <div id="adj-current-box" class="info-box">
      <span class="cell-soft">المخزون الحالي</span>
      <strong id="adj-cur-val" class="text-lg">—</strong>
    </div>
    ...
    <div class="form-group">
      ...
      <input type="number" id="adj-qty" class="form-input input-lg-compact" step="0.001" min="0" placeholder="0" oninput="calcAdjPreview()">
    </div>

    <div id="adj-preview" class="u-style-53">
      <div class="u-style-26">
        <span class="u-style-54">المخزون الجديد</span>
        <strong id="adj-new-val" class="u-style-55">—</strong>
      </div>
    </div>
...
function calcAdjPreview() {
  ...
  document.getElementById('adj-new-val').style.color = nq<=0 ? 'var(--danger)' : (nq<adjCurQty && adjType!=='adjustment' ? 'var(--warning)' : 'var(--olive-600)');
...
async function adjSearch(q) {
  ...
  el.innerHTML = f.map(p => `<div onclick="selectAdjProd(${p.id},'${p.name.replace(/'/g,"\\'")}',${p.stock_qty})" class="u-style-56">${p.name} <span class="u-style-57">(${p.stock_qty} ${p.unit||''})</span></div>`).join('') || '<div class="u-style-58">لا نتائج</div>';
